...
pacman -S --noconfirm networkmanager git sudo nano base-devel bluez bluez-utils \
    ...
    power-profiles-daemon tlp ufw cups hplip system-config-printer cpupower \
    greetd wayland xdg-desktop-portal-wlr xorg-xwayland firefox
# Enable Services
...
# Check greetd and Hyprland configurations before rebooting
...
# Verify greetd config file
if [ ! -f /etc/greetd/config.toml ]; then
    echo "/etc/greetd/config.toml file is missing!"
    exit 1
else
    echo "greetd config file exists."
fi
...
# Check Wayland components
echo "Checking Wayland components..."
if ! pacman -Qi pipewire | grep -q 'Installed'; then
